...
from gym_moving_dot.envs.moving_dot_env import MovingDotContinuousEnv
...
if __name__ == "__main__":
    #### Save directory ########################################
    filename = os.path.dirname(os.path.abspath(__file__))+'/results/save-'+"trajtrack"+'-'+ALGO+'-'+datetime.now().strftime("%m.%d.%Y_%H.%M.%S")
    ...
    env_name = ENV+"-v0"
    register_env(env_name, lambda _: MovingDotContinuousEnv())
    #### Unused env to extract the act and obs spaces ##########
    temp_env = MovingDotContinuousEnv()
