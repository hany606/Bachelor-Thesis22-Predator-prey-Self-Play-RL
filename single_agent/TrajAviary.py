# Reference: https://github.com/utiasDSL/gym-pybullet-drones/blob/master/gym_pybullet_drones/envs/single_agent_rl/TuneAviary.py
import numpy as np
...
class TrajAviary(BaseSingleAgentAviary):
    ...
    def __init__(self,
        ...
        self.TARGET_POSITION = np.array([[0, 4.0*np.sin(0.006*self.TRAJ_STEPS), 1.0] for i in range(self.TRAJ_STEPS)]) #Test for fixed point # Traj for fixed trajectory: np.array([[0, 4.0*np.sin(0.006*i), 1.0] for i in range(self.TRAJ_STEPS)])
